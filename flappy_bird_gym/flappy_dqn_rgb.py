...
class DQNAgentRGB:
    def __init__(self, action_dim, learning_rate=1e-4, gamma=0.99, epsilon_start=1.0, epsilon_min=0.01,
                 epsilon_decay=0.995, model_save_path=os.path.join(Path.cwd().parent ,  "pickle_files" , "dqn_rgb_model.pth")):
        ...
        if os.path.exists(self.model_save_path):
            self.policy_net.load_state_dict(torch.load(self.model_save_path))
            self.target_net.load_state_dict(self.policy_net.state_dict())
            print("Model loaded from", self.model_save_path)
...
def train_dqn_agent_rgb():
    ...
    for episode in range(num_episodes):
        ...
        if episode % target_update_freq == 0:
            ...
            agent.save_model()  # Save model every target update
