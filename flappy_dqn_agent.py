...
def create_dqn(input_dim, output_dim):
    return nn.Sequential(
        ...
        nn.ReLU(),
        nn.Linear(256, output_dim)  # Output layer (Q-values for each action)
    )

class DQNAgent:
    def __init__(self, state_dim, action_dim, learning_rate=5e-4, gamma=0.99, epsilon_start=1.0, epsilon_min=0.1, epsilon_decay=0.9995):
